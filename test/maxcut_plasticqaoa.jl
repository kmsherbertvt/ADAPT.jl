...
This script is identical to `maxcut_diagoanlqaoa.jl`
    except that it uses the `PlasticQAOAAnsatz`,
    which uses `γ0` only for the first optimization,
    subsequently initializing `γ` with that of the previous layer.
...
ansatz = ADAPT.ADAPT_QAOA.PlasticQAOAAnsatz(0.1, pool, H)
