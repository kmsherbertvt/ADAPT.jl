...
import ADAPT
import PauliOperators: ScaledPauliVector, FixedPhasePauli, KetBitString, SparseKetBasis, PauliSum
...
# CONVERT POOL TO PAULI SUM
newpool = PauliSum{2L}[]
for op in pool
    newop = PauliSum(2L)
    for pauli in op; sum!(newop, pauli); end
    push!(newpool, newop)
end
pool = newpool
...
# ψ0 = zeros(ComplexF64, 1<<2L); ψ0[1+ket1.v] = 1
